...
sed -i "/uci commit network/i\uci set network.lan.ipaddr='192.168.123.1'" $ZZZ                       # IPv4 地址(openwrt后台地址)
sed -i "/uci commit network/i\uci set network.lan.netmask='255.255.255.0'" $ZZZ                    # IPv4 子网掩码
sed -i "/uci commit network/i\uci set network.lan.gateway='192.168.123.253'" $ZZZ                      # IPv4 网关
